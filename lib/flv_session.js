...
class NodeFlvSession {
  ...
  run() {
    ...
    console.log(`[${this.TAG} connect] id=${this.id} ip=${this.ip} args=${JSON.stringify(urlInfo.query)}`);
  ...
  stop() {
    if (this.isStarting) {
      ...
      console.log(`[${this.TAG} play] Close stream. id=${this.id} streamPath=${this.playStreamPath}`);
      console.log(`[${this.TAG} disconnect] id=${this.id}`);
  ...
  reject() {
    console.log(`[${this.TAG} reject] id=${this.id}`);
    this.stop();
  }
  ...
  onPlay() {
    ...
    if (this.config.auth !== undefined && this.config.auth.play) {
      ...
      if (!results) {
        console.log(`[${this.TAG} play] Unauthorized. id=${this.id} streamPath=${this.playStreamPath} sign=${this.playArgs.sign}`);
    ...
    if (!context.publishers.has(this.playStreamPath)) {
      console.log(`[${this.TAG} play] Stream not found. id=${this.id} streamPath=${this.playStreamPath} `);
  ...
  onStartPlay() {
    ...
    if (publisher.metaData !== null) {
      const packet = FlvPacket.create(publisher.metaData, 18);
      const tag = NodeFlvSession.createFlvTag(packet);
      this.res.write(tag);
    }

    // send aacSequenceHeader
    if (publisher.audioCodec === 10) {
      const packet = FlvPacket.create(publisher.aacSequenceHeader, 8);
      const tag = NodeFlvSession.createFlvTag(packet);
      this.res.write(tag);
    }
    ...
    if (publisher.videoCodec === 7 || publisher.videoCodec === 12) {
      const packet = FlvPacket.create(publisher.avcSequenceHeader, 9);
      const tag = NodeFlvSession.createFlvTag(packet);
      this.res.write(tag);
    }
    ...
    if (publisher.flvGopCacheQueue !== null) {
      for (const tag of publisher.flvGopCacheQueue) {
        this.res.write(tag);
      }
    }
    ...
    this.isIdling = false;
    this.isPlaying = true;
    console.log(`[${this.TAG} play] Join stream. id=${this.id} streamPath=${this.playStreamPath} `);
    context.nodeEvent.emit('postPlay', this.id, this.playStreamPath, this.playArgs);
  }

  static createFlvTag(packet) {
    const PreviousTagSize = 11 + packet.header.length;
