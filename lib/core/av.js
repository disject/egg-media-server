...
function HEVCParseSPS(SPS, hevc) {
  ...
  for (let i = 2; i < NumBytesInNALunit; i++) {
    if (i + 2 < NumBytesInNALunit && bitop.look(24) === 0x000003) {
      ...
      rbsp_array.push(bitop.read(8));
      i += 2;
      // const emulation_prevention_three_byte = bitop.read(8); /* equal to 0x03 */
    } else {
      rbsp_array.push(bitop.read(8));
    }
  }
  const rbsp = Buffer.from(rbsp_array);
  ...
  if (psps.conformance_window_flag) {
    const vert_mult = 1 + (psps.chroma_format_idc < 2);
    const horiz_mult = 1 + (psps.chroma_format_idc < 3);
...
function readAVCSpecificConfig(avcSequenceHeader) {
  const codec_id = avcSequenceHeader[0] & 0x0f;
  if (codec_id === 7) {
